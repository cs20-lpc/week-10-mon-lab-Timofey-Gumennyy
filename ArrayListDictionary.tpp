...
Val ArrayListDictionary<Key, Val>::binSearchIter(const Key& target, int left, int right) const {
    ...
    while (left <= right)
    {
        ...
        Record halfElement = list->getElement(half);
        numComps++;
        if (halfElement.k == target) return halfElement.v;
        if (target > halfElement.k) left = half + 1;
        else right = half - 1;
    }
    throw -1;
}
...
Val ArrayListDictionary<Key, Val>::binSearchRec(const Key& target, int left, int right) const {
    ...
    return binSearchIter(target, left, right);
}
...
Val ArrayListDictionary<Key, Val>::seqSearchIter(const Key& target) const {
    ...
    throw string("seqSearchIter: error, unable to find record with matching key");
}
